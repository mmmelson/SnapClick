...
最近偶然间惊喜地发现了[网页版的《红色警戒2》](https://game.chronodivide.com)，Mac 端也能玩，于是和童年记忆狠狠地链接了一下，很快就重新沉迷上了这款当年的启蒙游戏。

在游玩过程中，组建一个坦克大军通常需要频繁、快速地用鼠标点击同一个单位，有时候选错了也需要点击右键数十次来取消。特别是我只用触控板，手指简直酸爽。😅

我尝试过 GitHub 上的一些连点器工具，但要么需要付费，要么不够好用，只支持一种固定的点击配置。于是就有了这个项目。

**这就是 SnapClick：一个简单、稳定、功能强大的鼠标连点器。**
...
⚠️ **安全警告**：超过 200 CPS 的速率可能导致 macOS 不稳定或崩溃。应用强制执行 200 CPS 限制以保护您的系统。计算公式为：
...
**安全配置示例**：
- ✅ 100 次点击，1 秒完成 = 100 CPS（安全）
- ✅ 200 次点击，2 秒完成 = 100 CPS（安全）
- ❌ 100 次点击，0.5 秒完成 = 200 CPS（最大值，有风险）
- ❌ 100 次点击，0.3 秒完成 = 333 CPS（不允许）
